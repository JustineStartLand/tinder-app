...
const Sidebar = () => {
  ...
  const {
    getMyMatches,
    isFetchingMatches: isLoadingMyMatches,
    matches,
  } = useMatchStore()
    ...
    <>
      ...
      >
        <div className="flex flex-col h-full">
          {/* Header */}
          <div className="p-4 pb-[27px] border-b border-slate-200 flex justify-between items-center">
            <h2 className="text-xl font-bold text-slate-600">Matches</h2>
            <button
            ...
            {isLoadingMyMatches ? (
              ...
              matches.map((match) => (
                <Link key={match.id} to={`/chat/${match.id}`}>
                  <div className="flex items-center mb-4 cursor-pointer hover:bg-slate-50 p-2 rounded-lg transition-colors duration-300">
                    <img
      ...
      <button
        className="lg:hidden fixed top-4 left-4 p-2 bg-slate-500 text-white rounded-md z-0"
        onClick={toggleSidebar}
      >
...
const LoadingState = () => (
  <div className="flex flex-col items-center justify-center h-full text-center">
    <Loader className="text-slate-500 mb-4 animate-spin" size={48} />
